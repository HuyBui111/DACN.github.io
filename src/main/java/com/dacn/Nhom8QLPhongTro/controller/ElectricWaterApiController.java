...
public class ElectricWaterApiController {
    ...
    public ResponseEntity<List<ElectricWaterDto>> getAllElectricWatersByRentRoomId(@PathVariable Long id) {
        RentRoom rentRoom = rentRoomService.getRentRoomById(id).orElse(null);
        if(rentRoom == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        List<ElectricWater> electricWaters = electricWaterService.getAllElectricWatersByRentRoomWithAllStatus(rentRoom);
        if(electricWaters.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        return ResponseEntity.ok().body(electricWaters.stream().map(this::convertToElectricWaterDto).toList());
    }
    ...
    public ResponseEntity<ElectricWaterDto> getById(@PathVariable Long id){
        ...
        return ResponseEntity.ok().body(convertToElectricWaterDto(electricWater));
    }
