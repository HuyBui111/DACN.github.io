...
public class OtherServiceApiController {
    ...
    public ResponseEntity<List<OtherServiceDto>> getAllOtherServiceByRentRoomId(@PathVariable Long id) {
        RentRoom rentRoom = rentRoomService.getRentRoomById(id).orElse(null);
        if (rentRoom == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok().body(otherServiceService.getOtherServiceByRentRoomWithAllStatus(rentRoom).stream().map(this::convertToOtherServiceDto).toList());
    }
